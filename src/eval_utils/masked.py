...
def generate_outputs(self, model_eval, X, y, requires_grad, get_accuracy):
    ...
    if get_accuracy == True:
        ...
        top1_words = [self.tokenizer.decode([token.item()]) for token in top1_predictions]
        top10_words = [[self.tokenizer.decode([token.item()]) for token in tokens] for tokens in top_tokens]
